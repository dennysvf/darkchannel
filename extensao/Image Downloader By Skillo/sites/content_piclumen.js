// content_piclumen.js - Piclumen
// Site: https://piclumen.com

let processedImages = new Set();
let isInitialized = false;
let shouldStop = false;
const MAX_HISTORY = 10;
// ===== INÍCIO DO SCRIPT PICLUMEN =====
// Verificar se já foi carregado para evitar duplicatas
if (window.__PICLUMEN_SCRIPT_LOADED__) {
    console.log('[Piclumen] Script já carregado, ignorando duplicata');
} else {
    window.__PICLUMEN_SCRIPT_LOADED__ = true;
    console.log('[Piclumen] Script carregado pela primeira vez');
}

const SITE_NAME = "Piclumen";
...
async function initializeImageHistory() {
    if (!isInitialized) {
        ...
        // Piclumen: seletores específicos para imagens geradas
        const existingImages = document.querySelectorAll('img[src*="piclumen"], img[src*="generated"], div[class*="result"] img, div[class*="output"] img, canvas + img');
        ...
        chrome.storage.local.set({ 
            piclumen_processedImages: Array.from(processedImages),
            piclumen_isInitialized: true
        });
    }
}
...
// Piclumen: Seletores específicos
...
async function waitAndDownloadImages(prompt) {
    ...
    const maxWaitTime = 50000; // 50 segundos
    ...
    while (elapsedTime < maxWaitTime) {
        ...
        // Piclumen: seletores de imagens geradas
        const allImages = document.querySelectorAll('img[src*="piclumen"], img[src*="generated"], img[src*="blob:"], div[class*="result"] img, div[class*="output"] img, div[class*="image"] img');
        ...
        if (newImagesFound.length >= 4) {
            console.log(`[${SITE_NAME}] ✓ 4 imagens encontradas! Baixando...`);
            await sleep(200); // Reduzido de 500ms para 200ms!
...
async function downloadImages(imagesToProcess, prompt) {
    ...
    for (let j = 0; j < imagesToProcess.length; j++) {
        ...
        try {
            ...
            a.href = url;
            a.download = `piclumen_${cleanPrompt}_${timestamp}_${j + 1}.jpg`;
...
async function initOnPageLoad() {
    ...
    chrome.storage.local.get(['piclumen_isInitialized'], async (result) => {
        if (!result.piclumen_isInitialized) {
            console.log(`[${SITE_NAME}] Primeira carga, inicializando...`);
            await initializeImageHistory();
        } else {
            chrome.storage.local.get(['piclumen_processedImages'], (result) => {
                if (result.piclumen_processedImages) {
                    processedImages = new Set(result.piclumen_processedImages);
                    isInitialized = true;
